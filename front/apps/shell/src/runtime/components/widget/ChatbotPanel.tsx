...
const createWelcomeText = (language: Language) => {
  return language === "en"
    ? "Hello, I am your Jeju Group assistant"
    : "안녕 제주그룹 도우미 상태";
};
...
export const ChatbotPanel = ({ isOpen, onOpen, onClose, language, onLanguageChange }: ChatbotPanelProps) => {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const botMessage = data?.choices?.[0]?.message?.content ?? "응답 실패 상태";
    ...
    <>
      ...
      <div className={`chatbot-container ${isOpen ? "active" : ""}`}>
        ...
        <div className="chatbot-messages" ref={containerRef}>
          {messages.map((message) => (
            <div key={message.id} className={`message ${message.type}`}>
              ...
              <div className="message-time">{message.timestamp.toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })}</div>
            </div>
          ))}
          {loading ? (
        ...
        <form className="chatbot-input-area" onSubmit={onSubmit}>
          ...
          />
          <button type="submit" disabled={loading}>{language === "en" ? "Send" : "전송"}</button>
        </form>
      </div>
    </>
  );
